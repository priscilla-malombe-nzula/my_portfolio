// Project data — ordered: Manufacturing → HR → Business/Sales (core sectors) first, then others
const projects = [
    ...
    // ── MANUFACTURING ─────────────────────────────────────────────────────
    {
        ...
        businessProblem: "Unplanned aircraft engine failures cost airlines $500,000+ per incident. Traditional maintenance runs on fixed schedules rather than actual engine condition, leading to both premature replacements and unexpected failures. This project builds an end-to-end system that monitors live sensor data and flags engines before they reach a critical state.",
        approach: [
            "Ingested NASA CMAPSS FD001 dataset (20,631 sensor readings, 100 engines) into a normalized MySQL schema with 3 tables and ~80k rows",
            "Engineered rolling average and standard deviation features (20-cycle window) for sensors 2, 7, 11, 12 and a cycle_ratio degradation signal",
            "Trained a Random Forest Regressor (100 trees, max_depth=15) to predict Remaining Useful Life (RUL), capped at 125 cycles",
            "Deployed a live Streamlit dashboard connected to Supabase with automated daily email alerts via GitHub Actions"
        ],
        results: [
            "Achieved R² = 0.969, RMSE = 7.2 cycles, MAE = 4.4 cycles — predicting engine failure within ~7 cycles on average",
            "Identified 25 engines in the danger zone (RUL < 30), 20 in warning, 55 healthy across the 100-engine test fleet",
            "cycle_ratio dominates feature importance at 94.5%, confirming life-stage position as the strongest degradation signal",
    ...
    {
        id: 15,
        title: "StyleSense: Retail Sales & Customer Analytics",
        category: "business",
        featured: true,
        image: "images/monthly_rev-profit_trend.PNG",
        businessProblem: "Retail businesses struggle to understand which products drive revenue, which customers are most valuable, and where seasonal demand patterns create inventory risks. StyleSense addresses this by turning raw retail transaction data into actionable sales intelligence.",
        approach: [
            "Performed end-to-end retail EDA covering sales trends, product performance, customer segmentation, and seasonal patterns",
            "Built customer RFM (Recency, Frequency, Monetary) segmentation to identify high-value vs at-risk customer groups",
            "Developed revenue forecasting models and inventory demand predictions to support buying decisions",
            "Created an interactive analytics dashboard presenting KPIs, cohort analysis, and product-level insights"
        ],
        results: [
            "Identified top-performing product categories and seasonal peaks enabling smarter inventory planning",
            "Segmented customer base into actionable groups (Champions, Loyal, At-Risk, Lost) for targeted marketing campaigns",
            "Quantified revenue concentration risk: top 20% of customers driving 80%+ of total revenue",
...
// Render projects — featured cards get a banner badge
function renderProjects(category = 'all') {
    ...
    filteredProjects.forEach(project => {
        ...
        if (project.featured) projectCard.classList.add('project-featured');
        ...
        const demoLink     = project.demo   ? `<a href="${project.demo}"   target="_blank" class="btn btn-small btn-outline"><i class="fas fa-external-link-alt"></i> Live Demo</a>` : '';
        const featuredBadge = project.featured ? `<span class="featured-badge">Featured</span>` : '';

        const projectInitials = project.title.split(' ').map(w => w[0]).join('').toUpperCase().substring(0, 3);
        const imgHTML = project.image
            ? `<img src="${project.image}" loading="lazy" alt="${project.title}" style="width:100%;height:100%;object-fit:cover;display:block;">`
            : `<div role="img" aria-label="${project.title}" style="width:100%;height:100%;background-color:${project.imageColor};display:flex;align-items:center;justify-content:center;color:white;font-weight:600;font-size:1.5rem;">${projectInitials}</div>`;
...
function setHeaderHeight() {
    ...
    document.querySelectorAll('section, .hero').forEach(el => { el.style.scrollMarginTop = `${h}px`; });
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addEventListener('resize', () => {
        setHeaderHeight();
        if (!isMobileScreen() && navLinks.classList.contains('active')) closeMobileMenu();
    });

    if (isMobileScreen()) {
        navLinks.style.transition = 'none';
        setTimeout(() => { navLinks.style.transition = ''; }, 100);
    }
    navLinks.setAttribute('aria-hidden', 'true');
    ...
    document.querySelectorAll('.nav-links a').forEach(link => {
        link.addEventListener('click', () => { if (isMobileScreen()) closeMobileMenu(); });
    });

    document.addEventListener('click', e => {
        if (!isMobileScreen() || !navLinks.classList.contains('active')) return;
        if (!navLinks.contains(e.target) && !menuToggle.contains(e.target)) closeMobileMenu();
    });
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && navLinks.classList.contains('active')) { closeMobileMenu(); menuToggle.focus(); }
    });
    ...
    window.addEventListener('load', () => { setTimeout(setHeaderHeight, 100); });

    if (window.location.hash && window.location.hash !== '#') {
        const el = document.querySelector(window.location.hash);
        if (el) {
            setTimeout(() => {
                const h = document.querySelector('header')?.offsetHeight + 10 || 80;
                window.scrollTo({ top: el.offsetTop - h, behavior: 'smooth' });
            }, 100);
        }
    }
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (isMobileScreen() && navLinks.classList.contains('active')) {
                closeMobileMenu();
                setTimeout(() => window.scrollTo({ top: el.offsetTop - h, behavior: 'smooth' }), 300);
            } else {
                window.scrollTo({ top: el.offsetTop - h, behavior: 'smooth' });
            }
        });
